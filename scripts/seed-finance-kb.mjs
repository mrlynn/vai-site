...
const DB_NAME = 'vai';
const COLLECTION_NAME = 'finance_chatbot';
const SLUG = 'finance';
const DOCS_DIR = join(import.meta.dirname, '..', 'public', 'use-cases', 'finance', 'sample-docs');
...
async function main() {
  console.log('📄 Reading finance markdown files...');
  const files = readdirSync(DOCS_DIR).filter(f => f.endsWith('.md'));
  console.log(`  Found ${files.length} files`);
  const allChunks = [];
  for (const file of files) { const content = readFileSync(join(DOCS_DIR, file), 'utf-8'); allChunks.push(...chunkDocument(content, file)); }
  console.log(`📦 Created ${allChunks.length} chunks`);
  console.log('🧠 Embedding chunks with voyage-finance-2...');
