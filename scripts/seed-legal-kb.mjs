#!/usr/bin/env node
/**
 * Seed script: Embed legal sample docs and store in MongoDB.
 * Usage: node scripts/seed-legal-kb.mjs
 */
...
const DB_NAME = 'vai';
const COLLECTION_NAME = 'legal_chatbot';
const SLUG = 'legal';
const DOCS_DIR = join(import.meta.dirname, '..', 'public', 'use-cases', 'legal', 'sample-docs');
...
async function main() {
  console.log('📄 Reading legal markdown files...');
  ...
  console.log('🧠 Embedding chunks with voyage-law-2...');
  ...
  try {
    ...
    try {
      ...
      if (e.message?.includes('already exists') || e.codeName === 'IndexAlreadyExists') {
        ...
        console.log('⚠️  Could not auto-create index:', e.message);
      }
    }
